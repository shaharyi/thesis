...
IMAGE_FILENAME      = 'kidney1.jpg' #'brain_t2_003.jpg' #
...
def sample(segments, interval):
    ...
    for lsegments in segments:
        for s in lsegments:
            mid,p0,p1,sl,sv = s
            times = sl/interval
            seg_samples = [meani(p0,p1,times,i) for i in range(times)]
...
def compute_rbf((Gx,Gy), segments, use_dist_trans=True):
    ...
    if RBF_SAMPLE_INTERVAL:
        pts = [p for l in segments for s in l for p in s[0:3]]
        sz = [v for l in segments for s in l for v in 3*[s[4]]]
        sx,sy = zip(*pts)
